...
def get_model(args):
    ...
    if 'Intern' in model_name:
        return InternVL2_5(model_name, temperature, max_new_tokens)
...
class InternVL2_5(nn.Module):
    ...
    def forward(self, head, prompts):
        messages = {}
        for p_f in prompts:
            prompt, frame_path = p_f
            messages["frame_path"] = frame_path
            messages["prompt"] = prompt

        generation_config = dict(max_new_tokens=self.max_new_tokens, do_sample=False, temperature=self.temperature)

        input_size = 448
        max_num = 1
        pixel_values_list, num_patches_list = [], []
        transform = self.build_transform(input_size=input_size)
        for f in messages["frame_path"]:
